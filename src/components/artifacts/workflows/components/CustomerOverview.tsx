...
const MetricCard = ({ metric }: { metric: CustomerMetric }) => {
  ...
  return (
    ...
    <span className={`text-xs ${
      ...
      {metric.sparkData && (
        <div className="flex items-end justify-center space-x-px mt-2">
...
const CustomerOverview: React.FC<CustomerOverviewProps> = ({ config, className = '' }) => {
    ...
    <div className={`flex-1 bg-white rounded-lg border border-gray-200 p-4 overflow-y-auto ${className}`}>
      <div className="h-full">
        <div className="grid grid-cols-2 grid-rows-4 gap-3 h-full">
          <MetricCard metric={metrics.arr} />
          <MetricCard metric={metrics.licenseUnitPrice} />
          ...
          <MetricCard metric={metrics.riskScore} />
