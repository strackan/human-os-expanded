...
/**
 * EnhancedSkipModal Component - Phase 1.1
 *
 * Skip modal with trigger-aware version.
 * Allows CSMs to configure date and event triggers for smart workflow reactivation.
 *
 * Features:
 * - Quick skip options (1 day, 3 days, 1 week, custom)
 ...
 * - Preview of reactivation conditions
 * - Validation and error handling
 */

import React, { useState } from 'react';
import { X, Clock, Sparkles, Zap, SkipForward } from 'lucide-react';
import { TriggerBuilder } from './triggers/TriggerBuilder';
import { SkipTrigger, DateTriggerConfig, TriggerLogic } from '@/types/skip-triggers';

// =====================================================
// Types
// =====================================================

export interface EnhancedSkipModalProps {
  workflowId: string;
  isOpen: boolean;
  onClose: () => void;
  onSkip: (triggers: SkipTrigger[], logic?: TriggerLogic, reason?: string) => Promise<void>;
  className?: string;
}

type SkipMode = 'date-only' | 'event-only' | 'date-and-events';
type QuickOption = '1day' | '3days' | '1week' | 'custom';

// =====================================================
// EnhancedSkipModal Component
// =====================================================

export const EnhancedSkipModal: React.FC<EnhancedSkipModalProps> = ({
  workflowId,
  isOpen,
  onClose,
  onSkip,
  ...
  const [mode, setMode] = useState<SkipMode>('date-only');
  const [quickOption, setQuickOption] = useState<QuickOption>('1day');
  const [customDate, setCustomDate] = useState('');
  const [customTime, setCustomTime] = useState('09:00');
  const [triggers, setTriggers] = useState<SkipTrigger[]>([]);
  const [triggerLogic, setTriggerLogic] = useState<TriggerLogic>('OR');
  const [skipReason, setSkipReason] = useState('');
  const [isProcessing, setIsProcessing] = useState(false);
  const [error, setError] = useState('');

  console.log('[EnhancedSkipModal] Render - isOpen:', isOpen, 'workflowId:', workflowId);

  if (!isOpen) {
    console.log('[EnhancedSkipModal] Not open - returning null');
    return null;
  }

  console.log('[EnhancedSkipModal] Modal is open - rendering...');
  ...
  const handleSkip = async () => {
    ...
    try {
      ...
      console.log('[EnhancedSkipModal] Skipping with triggers:', finalTriggers, 'logic:', finalLogic);
      await onSkip(finalTriggers, finalLogic, skipReason || undefined);
      onClose();
    } catch (err: any) {
      setError(err.message || 'Failed to skip workflow');
    } finally {
      setIsProcessing(false);
    }
  };
  ...
  const getPreviewText = (): string => {
    if (mode === 'date-only') {
      ...
      return `Reactivate on ${dateStr}`;
    } else if (mode === 'event-only') {
      if (triggers.length === 0) {
        return 'Reactivate when [add triggers below]';
      }
      const eventDescriptions = triggers
        .map((t: any) => t.config.eventType.replace(/_/g, ' '))
        .join(triggerLogic === 'OR' ? ' OR ' : ' AND ');
      return `Reactivate when: ${eventDescriptions}`;
    } else {
      // date-and-events
      const date = getDateFromQuickOption();
      const dateStr = date.toLocaleDateString('en-US', {
      ...
      if (triggers.length === 0) {
        return `Reactivate on ${dateStr} ${triggerLogic} when [add triggers below]`;
      }
      ...
      if (triggerLogic === 'OR') {
        return `Reactivate on ${dateStr} OR when: ${eventDescriptions}`;
      } else {
        return `Reactivate when date reached (${dateStr}) AND: ${eventDescriptions}`;
      }
    }
  };
      ...
      <div className={`bg-white rounded-lg shadow-xl p-6 max-w-2xl w-full mx-4 relative max-h-[90vh] overflow-y-auto ${className}`}>
        ...
        <div className="space-y-6">
            ...
            <div className="space-y-2">
              <label className="flex items-center cursor-pointer p-3 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">
                ...
                <div>
                  ...
                  <div className="text-xs text-gray-500">
                    Reactivate at a specific date and time
                  </div>
                </div>
              </label>
              ...
              <label className="flex items-center cursor-pointer p-3 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">
                ...
                <div className="flex-1">
                  ...
                  <div className="text-xs text-gray-500">
                    Reactivate when specific conditions are met (no date required)
                  </div>
                </div>
              </label>
          ...
          {showDatePicker && (
            <>
              <div>
                <label className="block text-sm font-medium text-gray-900 mb-2">
                  {mode === 'date-and-events' ? 'Date trigger:' : 'How long should this workflow remain skipped?'}
                </label>
          ...
          {showEventBuilder && (
            <div className="border border-purple-200 rounded-lg p-4 bg-purple-50">
              <h4 className="text-sm font-semibold text-gray-900 mb-3">
                {mode === 'event-only' ? 'Reactivation Conditions' : 'Additional Reactivation Conditions'}
              </h4>
              <TriggerBuilder
            ...
            <div>
              ...
              <div className="flex gap-2">
                ...
                >
                  ...
                  <div className="text-xs mt-1">Reactivate when ANY condition is met</div>
                </button>
                <button
                  type="button"
                ...
                >
                  ...
                  <div className="text-xs mt-1">Reactivate only when ALL conditions are met</div>
                </button>
              </div>
            </div>
          )}
        ...
        <div className="flex justify-end gap-3 mt-6">
          ...
          >
            ...
            {isProcessing ? 'Skipping...' : 'Skip Workflow'}
          </button>
        </div>
      </div>
    </div>
