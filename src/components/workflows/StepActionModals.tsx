 ...
 * Step Action Modals
 *
 * Snooze, Skip, and Review modals for individual workflow steps
 */
...
export function StepReviewModal({
  ...
  const handleReview = async () => {
    ...
    try {
      const service = new WorkflowStepActionService();
      const result = await service.requestStepReview(
        executionId,
        stepIndex,
        stepId,
        stepLabel,
      ...
      if (result.success) {
        ...
        setError(result.error || 'Failed to request review');
      }
    } catch (err: any) {
      setError(err.message);
    } finally {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-20 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl p-6 max-w-md w-full mx-4 relative">
        ...
        <h3 className="text-lg font-semibold mb-4 text-gray-900 flex items-center gap-2">
          ...
          Request Review: {stepLabel}
        </h3>

        <p className="text-sm text-gray-600 mb-4">
          Request review for this step before proceeding. You'll keep ownership but won't be able to complete this step until approved.
        </p>

        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-900 mb-1">
              Request review from: <span className="text-red-500">*</span>
            </label>
            <select
              value={reviewerId}
          ...
          <div>
            ...
            <textarea
              value={reason}
              onChange={(e) => setReason(e.target.value)}
              placeholder="e.g., Need approval for pricing decision"
              rows={3}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            />
          </div>
        ...
        <div className="flex justify-end gap-3 mt-6">
          ...
          <button
            ...
            disabled={isProcessing || !reviewerId}
            className="px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700 disabled:opacity-50 flex items-center gap-2"
          >
            <Shield className="w-4 h-4" />
            {isProcessing ? 'Requesting...' : 'Request Review'}
          </button>
        </div>
      </div>
    </div>
