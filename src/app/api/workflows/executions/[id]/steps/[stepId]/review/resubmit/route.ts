/**
 * Resubmit Step After Rejection API
 *
 * POST /api/workflows/executions/[id]/steps/[stepId]/review/resubmit
 * - Re-submit a rejected workflow step for review
 *
...
export async function POST(
  ...
  try {
    ...
    await reviewService.resubmitStep(params.id, params.stepId, userId);

    return NextResponse.json({
      success: true,
      message: 'Step re-submitted successfully',
      executionId: params.id,
      stepId: params.stepId
    });

  } catch (error) {
    console.error('[resubmit-step-review] Error:', error);
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Failed to re-submit step' },
      { status: 500 }
    );
  }
}
