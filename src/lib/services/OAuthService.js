...
export class OAuthService {
     ...
     */
    static async getAuthorizationUrl(provider, integrationSlug, userId, redirectUri) {
     ...
     */
    static async exchangeCodeForTokens(provider, integrationSlug, code, redirectUri) {
        const config = OAUTH_CONFIGS[provider];
        if (!config) {
            throw new Error(`Unsupported OAuth provider: ${provider}`);
        }
     ...
     */
    static async refreshAccessToken(provider, integrationSlug, refreshToken) {
        const config = OAUTH_CONFIGS[provider];
        if (!config) {
            throw new Error(`Unsupported OAuth provider: ${provider}`);
        }
     ...
     */
    static async storeTokens(userIntegrationId, userId, credentials) {
        const supabase = createServiceRoleClient();
        ...
        const { data: encryptedAccess, error: accessError } = await supabase.rpc('encrypt_oauth_token', {
     ...
     */
    static async getTokens(userIntegrationId) {
        const supabase = createServiceRoleClient();
        ...
        const { data: accessToken, error: accessError } = await supabase.rpc('decrypt_oauth_token', {
     ...
     */
    static async getValidAccessToken(userIntegrationId, provider, integrationSlug) {
        ...
        if (isExpired && tokens.refresh_token) {
            // Refresh the token
            const newCredentials = await this.refreshAccessToken(provider, integrationSlug, tokens.refresh_token);
            // Store new tokens
     ...
     * Generate state parameter for CSRF protection
     */
    static generateState(userId, integrationSlug) {
        const data = {
            userId,
            integrationSlug,
            timestamp: Date.now(),
     ...
     */
    static getClientId(provider, integrationSlug) {
     ...
     */
    static getClientSecret(provider, integrationSlug) {
