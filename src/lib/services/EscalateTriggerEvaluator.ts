/**
 * Escalate Trigger Evaluator Service
 *
 * Core service for evaluating workflow escalate triggers.
...
import {
  ...
  isDateEscalateTrigger,
  isEventEscalateTrigger,
...
export class EscalateTriggerEvaluator {
  /**
  ...
  static async evaluateTrigger(
    ...
    try {
      ...
      if (isDateEscalateTrigger(trigger)) {
      ...
      } else if (isEventEscalateTrigger(trigger)) {
      ...
      console.error(`[EscalateTriggerEvaluator] Error evaluating trigger ${trigger.id}:`, errorMessage);
  ...
  static async evaluateDateTrigger(
    ...
    try {
      ...
      console.error('[EscalateTriggerEvaluator] Error evaluating date trigger:', error);
      return false;
    }
  }
  ...
  static async evaluateEventTrigger(
    ...
    try {
      ...
      console.error('[EscalateTriggerEvaluator] Error evaluating event trigger:', error);
      return {
        triggered: false,
  ...
  private static async evaluateCustomerLogin(
    ...
    try {
      ...
      const { data: workflow, error: workflowError } = await supabase
        ...
        .select('customer_id, escalate_last_evaluated_at')
      ...
      const lastEvaluatedDate = workflow.escalate_last_evaluated_at
        ? new Date(workflow.escalate_last_evaluated_at)
  ...
  private static async evaluateManualEvent(
    ...
    try {
      // Check workflow_escalate_triggers table for a fired manual event
  ...
  static async evaluateAllTriggers(
    ...
    const evaluationResults: Array<{ trigger: EscalateTrigger; result: TriggerEvaluationResult }> = [];
    let firedTrigger: EscalateTrigger | undefined;
  ...
  static async updateWorkflowWithEvaluationResults(
    ...
    if (shouldNotify && firedTrigger) {
      ...
      await supabase
        .from(DB_TABLES.WORKFLOW_EXECUTIONS)
        .update({
          escalate_last_evaluated_at: now,
          escalate_trigger_fired_at: now,
          escalate_fired_trigger_type: firedTrigger.type
        })
        .eq(DB_COLUMNS.ID, workflowExecutionId);
    } else {
  ...
  static async logTriggerEvaluation(
    ...
    try {
      ...
      console.error('[EscalateTriggerEvaluator] Error logging trigger evaluation:', error);
