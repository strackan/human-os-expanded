...
export class GmailService {
    ...
    static async getAccessToken(userId) {
        ...
        return await OAuthService.getValidAccessToken(userIntegrationId, 'google', 'gmail');
    }
    /**
     ...
     */
    static async sendEmail(userId, params) {
        // Build email headers
        const headers = [];
        // To addresses
     ...
     */
    static async listMessages(userId, query, maxResults = 10, pageToken) {
        ...
        return await this.apiRequest(userId, `/users/me/messages?${params.toString()}`);
    }
    /**
     ...
     */
    static async getMessage(userId, messageId, format = 'full') {
        const params = new URLSearchParams({
            format,
        });
        return await this.apiRequest(userId, `/users/me/messages/${messageId}?${params.toString()}`);
    }
    /**
     ...
     * @param messageId - Message ID
     */
    static async trashMessage(userId, messageId) {
        return await this.apiRequest(userId, `/users/me/messages/${messageId}/trash`, {
            method: 'POST',
        });
    }
    /**
    ...
    static async deleteMessage(userId, messageId) {
        await this.apiRequest(userId, `/users/me/messages/${messageId}`, {
            method: 'DELETE',
        });
    }
    /**
     ...
     */
    static async modifyLabels(userId, messageId, addLabelIds, removeLabelIds) {
        return await this.apiRequest(userId, `/users/me/messages/${messageId}/modify`, {
            method: 'POST',
            body: JSON.stringify({
                addLabelIds,
                removeLabelIds,
     ...
     * @param messageId - Message ID
     */
    static async markAsUnread(userId, messageId) {
