...
export async function POST(
  ...
  try {
    ...
    // Only allow unpublishing published entries
    if (entry.status.status !== 'Published') {
      return NextResponse.json({ 
        error: 'Only published entries can be unpublished' 
      }, { status: 400 });
    }
    ...
    // Update the entry status to draft while preserving publishedDate
    const updatedEntry = await prisma.entry.update({
      where: { id: entryId },
      data: {
        statusId: draftStatus.id,
        updatedDate: new Date()
        // Note: publishedDate is preserved (not changed)
      },
      include: {
        status: true,
        entryProps: true
    ...
    return NextResponse.json({
      ...
      entry: {
        ...
        publishedDate: updatedEntry.publishedDate?.toISOString() || null
      }
    });

  } catch (error) {
    console.error('Unpublish entry error:', error);
    return NextResponse.json({ 
