/**
 * Review Trigger Evaluator Service
 *
 * Core service for evaluating workflow review triggers.
...
// =====================================================
// ReviewTriggerEvaluator Service
// =====================================================

export class ReviewTriggerEvaluator {
  /**
  ...
  static async evaluateTrigger(
    ...
    try {
      ...
      if (isDateReviewTrigger(trigger)) {
      ...
      } else if (isEventReviewTrigger(trigger)) {
      ...
      console.error(`[ReviewTriggerEvaluator] Error evaluating trigger ${trigger.id}:`, errorMessage);
  ...
  static async evaluateDateTrigger(
    ...
    try {
      ...
      console.error('[ReviewTriggerEvaluator] Error evaluating date trigger:', error);
      return false;
    }
  }
  ...
  static async evaluateEventTrigger(
    ...
    try {
      ...
      console.error('[ReviewTriggerEvaluator] Error evaluating event trigger:', error);
      return {
        triggered: false,
  ...
  private static async evaluateCustomerLogin(
    ...
    try {
      ...
      const { data: workflow, error: workflowError } = await supabase
        ...
        .select('customer_id, review_last_evaluated_at')
      ...
      const lastEvaluatedDate = workflow.review_last_evaluated_at
        ? new Date(workflow.review_last_evaluated_at)
  ...
  private static async evaluateManualEvent(
    ...
    try {
      // Check workflow_review_triggers table for a fired manual event
      const { data, error } = await supabase
        .from(DB_TABLES.WORKFLOW_ESCALATE_TRIGGERS)
  ...
  static async evaluateAllTriggers(
    ...
    const evaluationResults: Array<{ trigger: ReviewTrigger; result: TriggerEvaluationResult }> = [];
    let firedTrigger: ReviewTrigger | undefined;
  ...
  static async updateWorkflowWithEvaluationResults(
    ...
    if (shouldNotify && firedTrigger) {
      // Trigger fired - record the trigger fire (notification handled by cron job)
      await supabase
        .from(DB_TABLES.WORKFLOW_EXECUTIONS)
        .update({
          review_last_evaluated_at: now,
          review_trigger_fired_at: now,
          review_fired_trigger_type: firedTrigger.type
        })
        .eq(DB_COLUMNS.ID, workflowExecutionId);
    } else {
  ...
  /**
   * Log trigger evaluation to workflow_review_triggers table
  ...
  static async logTriggerEvaluation(
    ...
    try {
      ...
      console.error('[ReviewTriggerEvaluator] Error logging trigger evaluation:', error);
