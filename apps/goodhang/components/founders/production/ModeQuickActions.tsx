...
import type { ProductionMode, QuickAction } from '@/lib/founders/types';
...
export function ModeQuickActions({ mode, onAction, disabled }: ModeQuickActionsProps) {
  ...
  return (
    <div className="flex gap-2 px-4 pb-2 overflow-x-auto scrollbar-hide">
      {actions.map((action) => (
        <button key={action.value} onClick={() => onAction(action.value)} disabled={disabled}
          className="flex-shrink-0 px-3 py-1.5 text-xs font-medium text-gray-400 bg-[var(--gh-dark-700)] hover:bg-[var(--gh-dark-600)] hover:text-white rounded-full border border-[var(--gh-dark-600)] transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
          {action.label}
        </button>
      ))}
    </div>
