...
import type { CodeContext } from '../index.js';
...
When the task is complete, close the issue with this EXACT format (for artifact parsing):
...
## Artifacts
<!-- FORMAT: path|language|lines - one per line -->
hello.py|python|42
src/utils.ts|typescript|128
...
IMPORTANT: List ALL created/modified files in the Artifacts section using the format: path|language|lines
...
/**
 * Parse artifacts from a closing comment
 * Format: path|language|lines (one per line in ## Artifacts section)
 */
function parseArtifacts(commentBody: string): Array<{ path: string; language: string; lines: number }> {
  const artifacts: Array<{ path: string; language: string; lines: number }> = [];

  // Find the Artifacts section
  const artifactsMatch = commentBody.match(/## Artifacts\s*\n(?:<!--[^>]*-->\s*\n)?([\s\S]*?)(?=\n##|$)/i);
  if (!artifactsMatch || !artifactsMatch[1]) return artifacts;

  const artifactsSection = artifactsMatch[1];
  const lines = artifactsSection.split('\n').filter(line => line.trim() && !line.startsWith('<!--'));
  ...
  for (const line of lines) {
    ...
    if (parts.length >= 3 && parts[0] && parts[1] && parts[2]) {
      artifacts.push({
        ...
        lines: parseInt(parts[2].trim(), 10) || 0,
      });
    }
  }
...
async function codeStatus(
  ...
  try {
    ...
    // Parse artifacts if task is complete
    let artifacts: Array<{ path: string; language: string; lines: number; githubUrl?: string }> | undefined;

    if (issue.state === 'CLOSED' && latestComment?.body) {
      const parsedArtifacts = parseArtifacts(latestComment.body);

      if (parsedArtifacts.length > 0 && execution?.branch_name) {
        // Get repo info for GitHub URLs
        try {
          const { stdout: repoJson } = await exec('gh repo view --json owner,name');
          const repo = JSON.parse(repoJson);

          artifacts = parsedArtifacts.map(artifact => ({
            ...artifact,
            githubUrl: `https://github.com/${repo.owner.login}/${repo.name}/blob/${execution.branch_name}/${artifact.path}`,
          }));
          ...
          await ctx
            ...
            .schema(DB_SCHEMAS.HUMAN_OS)
            .from('code_executions')
            .update({ artifacts: parsedArtifacts })
            .eq('id', execution.id);
        } catch {
          // If we can't get repo info, return artifacts without URLs
 ...
 */
async function codeList(ctx: CodeContext): Promise<{
  count: number;
  tasks: Array<{
    issueNumber: number;
    title: string;
