 ...
 * Tool Execution Card
 *
 * Inline card shown in chat when do() matches an alias and executes a tool.
 * Shows tool name, brief result summary, and status indicator.
 */

import { motion } from 'framer-motion';
import { CheckCircle, AlertCircle, AlertTriangle } from 'lucide-react';
import type { DoGateResult } from '@/lib/types/production';
...
export function ToolExecutionCard({ result }: ToolExecutionCardProps) {
  ...
  return (
    <motion.div
      ...
      className={`flex items-start gap-3 px-4 py-3 rounded-lg border ${statusConfig.color} mx-4 my-1`}
    >
      <span className={`mt-0.5 ${statusConfig.iconColor}`}>
        {statusConfig.icon}
      </span>
