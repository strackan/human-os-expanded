 ...
 * Production Sidebar
 *
 * Vertical sidebar with mode buttons and quick access actions.
 * Collapsible between icon-only (60px) and expanded (200px).
 */
...
const MODE_CONFIG: Array<{
  ...
  { mode: 'checkin', label: 'Check-in', icon: <Heart className="w-5 h-5" />, accent: 'text-green-400 bg-green-400/10 border-green-400/30' },
...
export function ProductionSidebar({
  ...
  return (
    <motion.aside
      ...
      className="flex flex-col border-r border-gh-dark-700 bg-gh-dark-850 h-full overflow-hidden"
    >
      {/* Collapse toggle */}
      ...
      <div className="flex flex-col gap-1 px-2">
        {!collapsed && (
          <span className="text-[10px] uppercase tracking-wider text-gray-600 px-2 mb-1">
            Modes
          </span>
        )}
          ...
          return (
            <button
              ...
              className={`flex items-center gap-3 rounded-lg transition-all ${
                ...
                isActive
                  ? `${accent} border`
                  : 'text-gray-400 hover:text-white hover:bg-gh-dark-700 border border-transparent'
              }`}
            >
              {icon}
              {!collapsed && (
      ...
      {/* Divider */}
      <div className="my-3 mx-3 border-t border-gh-dark-700" />
      ...
      <div className="flex flex-col gap-1 px-2">
        ...
        {QUICK_ACCESS.map(({ id, label, icon }) => (
          <button
            key={id}
            title={label}
            className={`flex items-center gap-3 rounded-lg text-gray-400 hover:text-white hover:bg-gh-dark-700 transition-colors ${
              collapsed ? 'justify-center p-2.5' : 'px-3 py-2'
            }`}
          >
            {icon}
