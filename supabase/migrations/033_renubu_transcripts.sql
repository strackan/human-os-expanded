...
CREATE TABLE IF NOT EXISTS renubu.transcripts (
...
CREATE INDEX IF NOT EXISTS idx_renubu_transcripts_tenant ON renubu.transcripts(tenant_id);
CREATE INDEX IF NOT EXISTS idx_renubu_transcripts_date ON renubu.transcripts(call_date DESC);
CREATE INDEX IF NOT EXISTS idx_renubu_transcripts_type ON renubu.transcripts(call_type);
CREATE INDEX IF NOT EXISTS idx_renubu_transcripts_topics ON renubu.transcripts USING GIN(key_topics);
CREATE INDEX IF NOT EXISTS idx_renubu_transcripts_tags ON renubu.transcripts USING GIN(context_tags);
CREATE INDEX IF NOT EXISTS idx_renubu_transcripts_entities ON renubu.transcripts USING GIN(entity_ids);

-- Full-text search on summary and title
CREATE INDEX IF NOT EXISTS idx_renubu_transcripts_fts ON renubu.transcripts
  USING GIN(to_tsvector('english', coalesce(summary, '') || ' ' || coalesce(title, '')));

-- Updated_at trigger
DROP TRIGGER IF EXISTS renubu_transcripts_updated_at ON renubu.transcripts;
CREATE TRIGGER renubu_transcripts_updated_at BEFORE UPDATE ON renubu.transcripts
...
DROP POLICY IF EXISTS "renubu_transcripts_tenant_access" ON renubu.transcripts;
...
DROP POLICY IF EXISTS "renubu_transcripts_service" ON renubu.transcripts;
