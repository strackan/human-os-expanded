...
CREATE TABLE IF NOT EXISTS founder_os.claude_queue (
...
CREATE INDEX IF NOT EXISTS idx_claude_queue_pending ON founder_os.claude_queue(user_id, created_at)
  WHERE status = 'pending';
CREATE INDEX IF NOT EXISTS idx_claude_queue_user ON founder_os.claude_queue(user_id);
CREATE INDEX IF NOT EXISTS idx_claude_queue_session ON founder_os.claude_queue(session_id)
...
DROP POLICY IF EXISTS "claude_queue_user_select" ON founder_os.claude_queue;
CREATE POLICY "claude_queue_user_select" ON founder_os.claude_queue
  FOR SELECT USING (user_id = auth.uid());

DROP POLICY IF EXISTS "claude_queue_user_insert" ON founder_os.claude_queue;
CREATE POLICY "claude_queue_user_insert" ON founder_os.claude_queue
  FOR INSERT WITH CHECK (user_id = auth.uid());

DROP POLICY IF EXISTS "claude_queue_user_update" ON founder_os.claude_queue;
CREATE POLICY "claude_queue_user_update" ON founder_os.claude_queue
  FOR UPDATE USING (user_id = auth.uid());

-- Service role can do everything (for MCP server)
DROP POLICY IF EXISTS "claude_queue_service_all" ON founder_os.claude_queue;
