...
-- =============================================================================
CREATE TABLE IF NOT EXISTS founder_os.projects (
...
CREATE INDEX IF NOT EXISTS idx_founder_os_projects_user_id ON founder_os.projects(user_id);
CREATE INDEX IF NOT EXISTS idx_founder_os_projects_status ON founder_os.projects(status);
CREATE INDEX IF NOT EXISTS idx_founder_os_projects_priority ON founder_os.projects(priority);
CREATE INDEX IF NOT EXISTS idx_founder_os_projects_slug ON founder_os.projects(slug);

-- =============================================================================
-- MILESTONES TABLE
-- Project milestones with ordering and completion tracking
-- =============================================================================
CREATE TABLE IF NOT EXISTS founder_os.milestones (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX IF NOT EXISTS idx_founder_os_milestones_project_id ON founder_os.milestones(project_id);
CREATE INDEX IF NOT EXISTS idx_founder_os_milestones_status ON founder_os.milestones(status);
CREATE INDEX IF NOT EXISTS idx_founder_os_milestones_order ON founder_os.milestones(project_id, order_index);

-- =============================================================================
-- PROJECT MEMBERS TABLE
-- Multi-user collaboration with roles
-- =============================================================================
CREATE TABLE IF NOT EXISTS founder_os.project_members (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  project_id UUID NOT NULL REFERENCES founder_os.projects(id) ON DELETE CASCADE,
  user_id UUID NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_founder_os_project_members_project_id ON founder_os.project_members(project_id);
CREATE INDEX IF NOT EXISTS idx_founder_os_project_members_user_id ON founder_os.project_members(user_id);

-- =============================================================================
-- PROJECT LINKS TABLE
-- Links projects to contacts, companies, goals, entities
-- =============================================================================
CREATE TABLE IF NOT EXISTS founder_os.project_links (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  project_id UUID NOT NULL REFERENCES founder_os.projects(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_founder_os_project_links_project_id ON founder_os.project_links(project_id);
CREATE INDEX IF NOT EXISTS idx_founder_os_project_links_linked ON founder_os.project_links(linked_type, linked_id);
...
ALTER TABLE founder_os.tasks
  ADD COLUMN IF NOT EXISTS project_id UUID REFERENCES founder_os.projects(id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS milestone_id UUID REFERENCES founder_os.milestones(id) ON DELETE SET NULL;

CREATE INDEX IF NOT EXISTS idx_founder_os_tasks_project_id ON founder_os.tasks(project_id);
CREATE INDEX IF NOT EXISTS idx_founder_os_tasks_milestone_id ON founder_os.tasks(milestone_id);

ALTER TABLE founder_os.goals
  ADD COLUMN IF NOT EXISTS project_id UUID REFERENCES founder_os.projects(id) ON DELETE SET NULL;

CREATE INDEX IF NOT EXISTS idx_founder_os_goals_project_id ON founder_os.goals(project_id);

-- =============================================================================
-- AUTO-UPDATE TRIGGERS
-- =============================================================================
DROP TRIGGER IF EXISTS update_founder_os_projects_updated_at ON founder_os.projects;
CREATE TRIGGER update_founder_os_projects_updated_at BEFORE UPDATE ON founder_os.projects
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();

DROP TRIGGER IF EXISTS update_founder_os_milestones_updated_at ON founder_os.milestones;
CREATE TRIGGER update_founder_os_milestones_updated_at BEFORE UPDATE ON founder_os.milestones
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
...
DROP POLICY IF EXISTS "Service role full access" ON founder_os.projects;
CREATE POLICY "Service role full access" ON founder_os.projects FOR ALL USING (true);
DROP POLICY IF EXISTS "Service role full access" ON founder_os.milestones;
CREATE POLICY "Service role full access" ON founder_os.milestones FOR ALL USING (true);
DROP POLICY IF EXISTS "Service role full access" ON founder_os.project_members;
CREATE POLICY "Service role full access" ON founder_os.project_members FOR ALL USING (true);
DROP POLICY IF EXISTS "Service role full access" ON founder_os.project_links;
...
-- Idempotent: checks if projects exist before inserting
-- =============================================================================
DO $$
DECLARE
  ...
  v_good_hang_id UUID;
BEGIN
  -- Get or create Renubu project
  SELECT id INTO v_renubu_id FROM founder_os.projects WHERE user_id = v_user_id AND slug = 'renubu';
  IF v_renubu_id IS NULL THEN
...
Make relationship management effortless with AI.')
  ...
  -- Get or create Human OS project
  SELECT id INTO v_human_os_id FROM founder_os.projects WHERE user_id = v_user_id AND slug = 'human-os';
  IF v_human_os_id IS NULL THEN
  ...
  -- Get or create PowerPak project
  SELECT id INTO v_powerpak_id FROM founder_os.projects WHERE user_id = v_user_id AND slug = 'powerpak';
  IF v_powerpak_id IS NULL THEN
  ...
  -- Get or create Good Hang project
  SELECT id INTO v_good_hang_id FROM founder_os.projects WHERE user_id = v_user_id AND slug = 'good-hang';
  IF v_good_hang_id IS NULL THEN
    INSERT INTO founder_os.projects (id, user_id, name, slug, description, status, priority)
    VALUES
  ...
  -- Insert milestones for Renubu (only if they don't exist)
  ...
  -- Insert milestones for Human OS (only if they don't exist)
